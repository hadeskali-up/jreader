...
@Composable private fun ZoomPage(p:Page,fit:ReaderFit,webtoon:Boolean){var scale by remember{mutableFloatStateOf(1f)};var x by remember{mutableFloatStateOf(0f)};var y by remember{mutableFloatStateOf(0f)};var retry by remember{mutableIntStateOf(0)};val src=p.localPath?:p.remoteUrl;key(retry){AsyncImage(src,"Page ${p.index+1}",Modifier.then(if(webtoon)Modifier.fillMaxWidth() else Modifier.fillMaxSize()).graphicsLayer{scaleX=scale;scaleY=scale;translationX=x;translationY=y}.pointerInput(Unit){detectTransformGestures{_,pan,zoom,_->scale=(scale*zoom).coerceIn(1f,5f);x+=pan.x;y+=pan.y}},contentScale=when(fit){ReaderFit.WIDTH->ContentScale.FillWidth;ReaderFit.CONTAIN->ContentScale.Fit;ReaderFit.FILL->ContentScale.Crop},onError={retry++})}}
