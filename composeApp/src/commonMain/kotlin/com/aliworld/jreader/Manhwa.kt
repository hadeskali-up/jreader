...
data class MdTag(val id:String,val name:String)
...
class MangaDexRepo {
 ...
 suspend fun tags():List<MdTag> = get("/manga/tag")["data"]?.jsonArray.orEmpty().mapNotNull{e->val o=e.jsonObject;val a=o["attributes"]?.jsonObject?:return@mapNotNull null;if(a["group"]?.jsonPrimitive?.contentOrNull!="genre")return@mapNotNull null;MdTag(o["id"]!!.jsonPrimitive.content,localized(a["name"]?.jsonObject))}.filter{it.name.isNotBlank()}.sortedBy{it.name}
 suspend fun manga(query:String="",tagId:String?=null):List<MdManga>{
  val root=get("/manga") { append("limit","30");append("originalLanguage[]","ko");append("includes[]","cover_art");append("order[followedCount]","desc");listOf("safe","suggestive","erotica","pornographic").forEach{append("contentRating[]",it)};if(query.isNotBlank())append("title",query);if(tagId!=null)append("includedTags[]",tagId) }
...
@Composable fun ManhwaBrowse(open:(String)->Unit){val repo=remember{MangaDexRepo()};var q by remember{mutableStateOf("")};var selected by remember{mutableStateOf<MdTag?>(null)};var tags by remember{mutableStateOf(emptyList<MdTag>())};var tick by remember{mutableIntStateOf(0)};var loading by remember{mutableStateOf(true)};var error by remember{mutableStateOf<String?>(null)};var data by remember{mutableStateOf(emptyList<MdManga>())};LaunchedEffect(tick){loading=true;error=null;try{if(tags.isEmpty())tags=repo.tags();data=repo.manga(q.trim(),selected?.id)}catch(e:Exception){error=e.message?:"MangaDex request failed"};loading=false};Column(Modifier.fillMaxSize().padding(16.dp)){Text("Manhwa",style=MaterialTheme.typography.headlineMedium,fontWeight=FontWeight.Bold);Text("Powered by MangaDex",color=MaterialTheme.colorScheme.primary);Row{OutlinedTextField(q,{q=it},Modifier.weight(1f),singleLine=true,label={Text("Search Korean-origin titles")});Button({tick++},Modifier.padding(start=8.dp)){Text("Go")}};LazyRow(horizontalArrangement=Arrangement.spacedBy(8.dp),modifier=Modifier.padding(vertical=8.dp)){item{FilterChip(selected==null,{selected=null;tick++},{Text("All genres")})};items(tags,key={it.id}){tag->FilterChip(selected==tag,{selected=tag;tick++},{Text(tag.name)})}};Row(verticalAlignment=Alignment.CenterVertically){Text("${data.size} results${selected?.let{" · ${it.name}"}?:""}",style=MaterialTheme.typography.bodySmall);if(selected!=null||q.isNotBlank())TextButton({selected=null;q="";tick++}){Text("Clear")}};when{loading->MdState("Loading MangaDex…");error!=null->MdState(error!!,"Retry"){tick++};data.isEmpty()->MdState("No manhwa found");else->LazyVerticalGrid(GridCells.Adaptive(155.dp),verticalArrangement=Arrangement.spacedBy(12.dp),horizontalArrangement=Arrangement.spacedBy(12.dp)){items(data,key={it.id}){m->ElevatedCard({open(m.id)}){AsyncImage(m.cover,m.title,Modifier.fillMaxWidth().height(210.dp),contentScale=ContentScale.Crop);Text(m.title,Modifier.padding(10.dp),fontWeight=FontWeight.SemiBold)}}}}}}
